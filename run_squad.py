...
def evaluate(args, model, tokenizer, prefix=''):
    ...
    3. Predict the answer: model.eval() -> model(**inputs)
    4. Compute predictions: write_predictions_multi() 
    5. Evaluate with the official SQuAD script: evaluate_on_squad() XXXXX
    '''
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            if args.model_type in ['xlnet', 'xlm']:
                ...
                result = RawResult_multi(unique_id    = unique_id,
    ...
    if args.model_type in ['xlnet', 'xlm']:
        ...
        write_predictions_multi(examples, features, all_results, args.n_best_size,
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    # 1) --predict_file 인자(JSON 형식의 평가 데이터 파일) 읽기: 질문+문단+정답
...
def main():
    ...
    if args.server_ip and args.server_port:
        ...
        '''
        Python Tools for Visual Studio Debugging
        Visual Studio Code(VS Code)와 같은 편집기에서 Python 애플리케이션을 원격으로 디버깅할 수 있게 해주는 패키지
        이 기능을 사용하면 코드 실행 중에 멈추어 변수 값을 확인하거나, 코드 흐름을 단계별로 추적
        이것은 특히 복잡한 스크립트나 대규모 데이터를 다룰 때 버그를 찾거나 코드의 동작을 이해하는 데 매우 유용
        '''
        import ptvsd
        print("Waiting for debugger attach")
        #주어진 IP 주소와 포트에 대해 디버거의 연결을 허용
        ptvsd.enable_attach(address=(args.server_ip, args.server_port), redirect_output=True)
        #실제 디버거가 연결될 때까지 실행을 일시 중지. 이를 통해 개발자는 원격 시스템에서 실행 중인 스크립트에 대해 디버깅 세션을 시작가능
    ...
    '''
    저장된 체크포인트에서 모델을 로드하고 평가 데이터셋에 대해 평가를 수행
    '''
